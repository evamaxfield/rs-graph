...
db-migrate target="dev":
	...
	sed -i.bak '' 's|REPLACE_SQLITE_DB_PATH|{{db_path}}-{{db_version}}-{{target}}.db|g' {{justfile_directory()}}/alembic.ini
	mkdir -p {{justfile_directory()}}/rs_graph/data/files
	-alembic revision --autogenerate -m "{{current_git_details}}"
	-alembic upgrade head
	sed -i.bak '' 's|{{db_path}}-{{db_version}}-{{target}}.db|REPLACE_SQLITE_DB_PATH|g' {{justfile_directory()}}/alembic.ini
	sleep 0.5
	git add -A
