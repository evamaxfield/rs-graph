...
def align_dependencies(
    ...
    Returns:
        Tuple of:
        - matched: Dict mapping mentioned software (original) -> (imported dependency original, score)
...
if __name__ == "__main__":
    ...
    imported_deps = {
        ...
        "matplotlib", "rapidfuzz", "scipy", "requests"
    }
    
    mentioned_software = {
        "Scikit-Learn", "Pandas", "Seaborn", 
        "NumPy", "SciPy", "Matplotlib",
        "FIJI", "ImageJ", "GraphPad Prism"  # Hardware/external tools not in code
    }
    ...
    imported_deps2 = {
        "sklearn", "sktime", "imblearn", "skimage",
        "pandas", "geopandas", "dask", "modin",
        "boto3", "requests"
    }
    mentioned_software2 = {
        "Scikit-Learn", "scikit-time", "imbalanced-learn", "scikit-image",
        "Pandas", "GeoPandas", "Dask",
        "R", "SPSS", "ImageJ"
    }
    ...
    # Demonstrate different scoring functions
    print("\n\nExample 4: Different scoring functions")
    print("=" * 60)
    
    test_imported = {"sklearn", "numpy_financial"}
    test_mentioned = {"Scikit-Learn", "NumPy"}
    
    print(f"Imported: {sorted(test_imported)}")
    print(f"Mentioned: {sorted(test_mentioned)}")
    
    for scorer_name, scorer_func in [
        ("ratio", fuzz.ratio),
        ("token_sort_ratio", fuzz.token_sort_ratio),
        ("token_set_ratio", fuzz.token_set_ratio),
        ("WRatio", fuzz.WRatio),
    ]:
        matched4, _, _, _ = align_dependencies(
            test_imported,
            test_mentioned,
            score_cutoff=70.0,
            scorer=scorer_func
        ...
        print(f"\nUsing {scorer_name}:")
        for mention, (imp, score) in sorted(matched4.items()):
            print(f"  {mention:20} <- {imp:15} (score: {score:.1f})")