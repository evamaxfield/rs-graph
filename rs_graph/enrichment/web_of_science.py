...
def _get_articles_from_grant_id_and_funder(
    grant_id: str,
    funder: str,
    wos_api_key: str,
) -> list[types.DocumentWithGrantInformation] | NullGrantArticleSearchResult | types.ErrorResult:
    try:
        ...
        response = requests.get(WEB_OF_SCIENCE_API_URL, params=api_params, headers=api_headers)
        ...
        for record in records["REC"]:
            ...
            if (
                ...
                or "identifier" not in record["dynamic_data"]["cluster_related"]["identifiers"]
            ):
                continue

            # Get the DOI from the identifiers
            article_identifiers = record["dynamic_data"]["cluster_related"]["identifiers"]["identifier"]
