...
"""Classification-model performance summary, plus ARMM held-out-test diagnostics (confusion
matrices, README-length performance, breakdown tables).
...
# (results.json relative path, metric key prefix)
...
def _load_model_metrics_row(entry: dict) -> dict | None:
    """Load one model's saved eval results into its "full test set" summary row (None when
    ...
    if not results_path.exists():
        print("  MISSING -- results.json not found at this path.\n")
        return None
    ...
    print(
        f"  precision={actual_precision:.4f}, recall={actual_recall:.4f}, f1={actual_f1:.4f}\n"
    )
...
def _armm_detail_metrics(
    ...
    return [
        ("per_field", "mean macro F1 across fields", field_f1_mean),
        ("per_field", "SD macro F1 across fields", field_f1_sd),
        (
            "per_field",
            f"lowest field macro F1 ({field_min['feature_value']})",
            float(field_min["macro_f1"]),
        ),
        ("per_period", "mean macro F1 across periods", period_f1_mean),
        ("per_period", "SD macro F1 across periods", period_f1_sd),
        (
            "per_period",
            f"lowest period macro F1 ({period_min['feature_value']})",
            float(period_min["macro_f1"]),
        ),
        ("per_source", "SoftCite-2025 binary F1", _source_binary_f1("softcite_2025")),
        ("per_source", "JOSS binary F1", _source_binary_f1("joss")),
        (
            "per_source",
        ...
        ("readme_length", "1601-3200 chars macro F1", _readme_bin_f1("1601-3200")),
        ("readme_length", "<=100 chars macro F1", _readme_bin_f1("<=100")),
    ]
...
def classification_models_table_verification(output_dir: Path = u.OUTPUT_DIR) -> None:
    ...
    period, dataset source, and README length (macro-averaged).
    """
    print("--- Classification models table ---\n")
    summary_rows = []
    for entry in CLASSIFICATION_MODELS_TABLE:
        summary_row = _load_model_metrics_row(entry)
        if summary_row is not None:
            summary_rows.append(summary_row)

    # ---- ARMM detail breakdown: per-field, per-period, per-source, and README-length. ----
    ...
    detail_metrics = _armm_detail_metrics(field_eval, period_eval, source_eval, readme_perf)

    print("ARMM detail metrics:")
    for detail_group, metric, actual in detail_metrics:
        print(f"  [{detail_group}] {metric}: {actual:.4f}")
    print()
...
def armm_model_diagnostics(output_dir: Path = u.OUTPUT_DIR) -> None:
    ...
    Build the ARMM held-out-test diagnostics.
      ...
      - the pooled confusion matrix (`Blues` cmap by standard convention);
      - per-field confusion matrices for the top-8 fields + the held-out "Other"/"Unknown"
        buckets, each panel annotated with n and macro F1;
      - the performance-by-README-length figure, from sci-soft-models'
