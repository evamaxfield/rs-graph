...
def main() -> None:
    ...
    # Take a sample of 20 pairs for manual inspection per 10 points of predictive model confidence (0-10)
    sampled_rows = []
    for confidence_lower_bound in range(50, 100, 10):
        confidence_upper_bound = confidence_lower_bound + 10
        float_confidence_lower_bound = confidence_lower_bound / 100
        float_confidence_upper_bound = confidence_upper_bound / 100
        sample = (
            merged.filter(
                (pl.col("predictive_model_confidence") >= float_confidence_lower_bound)
                & (pl.col("predictive_model_confidence") < float_confidence_upper_bound)
            )
            .sample(n=20, seed=42)
            .select(
