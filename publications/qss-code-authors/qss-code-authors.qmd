...
def _run_chi2_and_posthocs(
    ...
    if control_var is None:
        ...
        if chi2_result.pvalue < 0.05:
            print(f"Chi2: {chi2_result.statistic}, p: {chi2_result.pvalue}, n: {len(data)}")
        ...
        for control_val in data[control_var].unique():
            ...
            chi2_result = chi2_contingency(xtabs)
            if chi2_result.pvalue < 0.05:
                print(f"Chi2: {chi2_result.statistic}, p: {chi2_result.pvalue}, n: {len(control_val_subset)}")
                posthoc_results = _run_posthocs(
                    control_val_subset,
                    split_var=split_var,
                )
...
def create_multirow_html_table(dataframes_dict):
