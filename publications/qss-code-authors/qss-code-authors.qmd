...
%% sources %%
plos["PLOS (n~10,000 pairs)"] --> unfiltered[Unfiltered Article-Repository Pairs]
softwarex["SoftwareX (n~700 pairs)"] --> unfiltered
joss["JOSS (n~3,000 pairs)"] --> unfiltered
pwc["Papers with Code (n~180,000 pairs)"] --> unfiltered
...
<!-- TODO: include the difference in article-repository pairs possible from the very start of the dataset to what our final database resulted in as a solution to "github as a sole repository host" issue. Note: there will definitely be some article-repository pairs which didn't make it through the pipeline due to errors (like article not found in OpenAlex) but GitHub was a decent source of dropped pairs -->

Our labeling of article types (software article, research article, preprint) was based on the data source (PLOS, JOSS, SoftwareX, Papers with Code) and in the case of Papers with Code articles, our DOI resolution process and the document type available from OpenAlex. This approach may misclassify certain articles, especially those from Papers with Code (arXiv). A potentially more robust approach would involve classification of the repository itself following the recommendations of @researchsoftwareclassifications in breaking down repositories by their role in research (e.g., "Modeling, Simuluation, and Data Analysis", "Technology Research Software", and "Research Infrastructure Software"). However, there is currently no established automated method for performing this classification at scale.
...
@tbl-team-composition-counts provides detailed statistics on the composition of research teams across different domains, article types, and open access statuses. @fig-team-composition provides a visual representation of the mean and standard deviation of the count of team members by contributor type, providing clearer visual comparison across categories.
...
1. **Name Variations and Cultural Differences**: The model performed better with Anglosaxon names, while names from other cultures were more likely to be missed. This suggests a possible bias in the training data that favors certain name structures.
2. **Additional Unrelated Text in Names**: When usernames or display names contained longer phrases or unrelated words, the model tended to classify them as no-match, even if there were strong indicators of a match. For example, a username such as "awesome_data_scientist_john_d" paired with an author name "John Doe" might be missed due to the additional text in the username.
3. **Significant Differences Between Username and Display Name**: The model struggled when there were substantial differences between the username and author name, such as when a user chose and used an English name in their GitHub profile that differed significantly from their authorship name.
4. **Limited Information from Text Alone**: The annotators often had to look beyond the provided text, such as visiting linked websites, to make accurate classifications. The model, however, only had access to the text features, which likely contributed to missed matches.
