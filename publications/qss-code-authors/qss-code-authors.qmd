...
# Get best base model
best_base_model = exp_results.loc[
    best_model_idx, "Model"
]

# Get best feature set
best_feature_set = exp_results.loc[
    best_model_idx, "Optional Feats."
]

# Format best feature set
# Either a single word or "word 1 and word 2"
if ", " in best_feature_set:
    best_feature_set = best_feature_set.replace(", ", " and ")
```

- After extensive model comparison we find that fine-tuning from `{python} best_base_model` and including the developer's `{python} best_feature_set` achieves the best performance for author-developer matching.
    ...
    - analysis of feature importance (todo, running overnight)
