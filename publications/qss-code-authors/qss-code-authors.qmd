...
Further, while our data processing workflow began with ~60,000 possible article-repository pairs from PLOS, ~3,000 from JOSS, ~700 from SoftwareX, and ~180,000 from Papers with Code for a possible total of ~243,700 [^11], `rs-graph-v1` contained a total of `{python} print_ready_m2m_doc_repo_links_count` article-repository pairs. Many of the possible article-repository pairs were filtered out due to the linked repository not being hosted on GitHub, GitHub repositories not being found (either private, non-existent, or moved), or SemanticScholar or OpenAlex not indexing the article.
