...
FROM unique_doc_repo_links
JOIN document_contributor ON unique_doc_repo_links.document_id = document_contributor.document_id
JOIN researcher ON document_contributor.researcher_id = researcher.id
"""
...
FROM unique_doc_repo_links
JOIN repository_contributor ON unique_doc_repo_links.repository_id = repository_contributor.repository_id
JOIN developer_account ON repository_contributor.developer_account_id = developer_account.id
"""

with Session(db_conn) as session:
  n_article_repo_pairs = execute_count_query(session, total_doc_repo_pairs_query)
  n_authors = execute_count_query(session, total_authors_query)
  n_devs = execute_count_query(session, total_devs_query)

n_article_repo_pairs, n_authors, n_devs
```
...
data_source_stats_df = pd.DataFrame(data_source_stats).set_index("data_source").sort_values("n_article_repo_pairs", ascending=False)
...
JOIN topic AS topic ON document_topic.topic_id = topic.id
...
JOIN document_topic ON document.id = document_topic.document_id
JOIN topic AS topic ON document_topic.topic_id = topic.id
WHERE topic.field_name = '{field_name}'
AND document_topic.id = (
  SELECT dt.id
...
JOIN document_topic ON repository.id = document_topic.document_id
JOIN topic AS topic ON document_topic.topic_id = topic.id
WHERE topic.field_name = '{field_name}'
AND document_topic.id = (
  SELECT dt.id
...
JOIN developer_account ON repository_contributor.developer_account_id = developer_account.id
JOIN repository ON repository_contributor.repository_id = repository.id
JOIN document ON repository.id = document
WHERE document.document_type = '{doc_type}'
"""
...
doc_type_counts_df = pd.DataFrame(doc_type_stats).set_index("doc_type").sort_values("n_article_repo_pairs", ascending=False)
doc_type_counts_df
```
