...
Contemporary scientific research fundamentally depends on specialized software tools and computational methods [@edwards2013knowledge; @mayernik2017assessing; @howison2015currentandfuturemeasures]. These tools, from analysis scripts for data processing to infrastructure for data collection [@hasselbring2024researchsoftwarecategories], have become essential to modern research practices. Further, software continues to play and ever-expanding role in science by enabling reproducible research and large-scale experiments [@krafczyk2019scientificreproducbility; @Trisovic2021ALS], serving as a detailed log of research methodology [@ram2013git], and increasingly being shared alongside research articles as a crucial research artifact [@Cao2023TheRO; @trujillo2022penumbra].

Despite their importance, software contributions face persistent challenges in receiving appropriate scientific recognition [@muna2016astropyproblem]. Software contributors are often relegated to acknowledgments sections rather than warranting authorship [@olivier_philippe_2019_2585783], and this lack of formal credit can significantly impact career advancement within research organizations [@Carver2022ASO; @biagioli2014scientific]. The scientific community continues to grapple with developing appropriate systems for software citation and credit attribution that adequately reflect these contributions [@Merow2023BetterIA; @Westner2024CyclingOT; @Katz2020RecognizingTV].

Recent initiatives like the Contributor Roles Taxonomy (CRediT) have attempted to address these challenges by expanding scientific authorship criteria to include specialized contribution roles to authorship lists [@brand2015beyond]. Further, prior work using CRediT attributions and other similar systems has been used to understand the distribution of labor within research teams, including investigation into specialist and generalist roles [@Larivire2020InvestigatingTD; @lariviere2016contributorship; @sauermann2017authorship; @Li2023AreRC; @Lu2019CocontributorshipNA]. However, these frameworks remain largely centered on traditional author lists, which perpetuates existing problems, including historical and systematic bias, and relies heavily on self-reporting without external verification [@HAEUSSLER2013688; @gotzsche2007ghost; @chaoqun2021gendered].

The emergence of public code repositories alongside published research provides a unique opportunity to move beyond self-reported contribution data. These repositories typically employ version control systems that maintain detailed records of who contributes what code and when, providing a window into the actual patterns of software development in scientific research. Leveraging this rich transaction history, we developed a novel predictive model that enables systematic matching between scientific article authors and source code developer accounts.

By applying our predictive model across a corpus of `{python} len(doc_repo_links)` paired research articles and repositories, we uncover several striking patterns in the relationship between code contributions and scientific recognition. Our analysis of source-code repositories contribution histories reveals that approximately 25% of articles have non-author code-contributors. We find that code-contributing authors are associated with modest increases in article-level impact metrics (~4.5% increase in citations per code-contributing author), though these effects become statistically non-significant when controlling for domain, article type, and open access status. First authors are significantly more likely to be code contributors compared to other positions across all conditions tested. And, most notably, we find a clear negative relationship between coding frequency and scholarly impact: authors who contribute code more frequently show progressively lower h-indices compared to their non-coding peers, a pattern that persists when controlling for an author's number of total works, and, most common author-position, domain, and article type.

Taken together, the primary contributions of this article are:

1. a predictive model for matching authors with developer account information that overcomes challenges in fuzzy identity resolution;
2. a comprehensive dataset of linked authors and developers for `{python} len(doc_repo_links)` article-repository pairs;
3. preliminary analyses across this dataset that reveal key insights into the software development dynamics of research teams, including patterns of recognition, impact, and career implications for code contributors.

The remainder of this paper proceeds as follows. We first provide relevant background on scientific software development and recognition, introducing the specific hypotheses that guide our investigation. Next, we detail our data and methods, describing how we created a dataset of linked article-repository pairs, trained and evaluated our predictive model for entity matching, and applied our model across each article-repository pai. We then present our preliminary analysis of the data, focusing first on article/team-level dynamics before moving to individual-level patterns, formally accepting or rejecting each hypothesis based on our findings. We conclude with a discussion of the results, limitations of our work, and areas for future improvement.
