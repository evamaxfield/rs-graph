...
for _, row in docs_w_1_citation.iterrows():
    ...
    # Get the number of first authors
    n_first_authors = len(
        document_contributors.loc[
            (document_contributors["document_id"] == row["document_id"])
            & (document_contributors["position"] == "first")
        ]["researcher_id"].unique()
    )
    ...
    team_composition_rows.append(
        {
            ...
            "n_authors": n_authors,
            "n_first_authors": n_first_authors,
...
# Find out how many papers have multiple "first" authors
one_first_author_teams = len(team_comp_no_push_after_pub.loc[
    team_comp_no_push_after_pub["n_first_authors"] == 1
])
multiple_first_author_teams = len(team_comp_no_push_after_pub.loc[
    team_comp_no_push_after_pub["n_first_authors"] > 1
])
total_teams = len(team_comp_no_push_after_pub)
print_ready_one_first_author_teams = "{:,}".format(one_first_author_teams)
print_ready_multiple_first_author_teams = "{:,}".format(multiple_first_author_teams)
print_ready_one_first_author_teams_pct = round(
    (one_first_author_teams / total_teams) * 100, 1
)
print_ready_multiple_first_author_teams_pct = round(
    (multiple_first_author_teams / total_teams) * 100, 1
)
...
2. **Name Variations and Cultural Differences**: The model performed better with Anglosaxon names, while names from other cultures were more likely to be missed. This suggests possible bias in the training data and a clear area for future work.
