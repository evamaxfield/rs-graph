...
Contemporary scientific research fundamentally depends on specialized software tools and computational methods [@edwards2013knowledge; @mayernik2017assessing; @howison2015currentandfuturemeasures]. These include the analysis scripts for data processing and statistical testing, and research tools and computational infrastructure for data collection and storage [@hasselbring2024researchsoftwarecategories], which act as essential components of modern research practices. Software itself additionally plays multiple critical roles: enabling reproducible research and large-scale experiments [@krafczyk2019scientificreproducbility; @Trisovic2021ALS], serving as a detailed log of research methodology [@ram2013git], and increasingly being shared alongside research articles as a crucial research artifact [@Cao2023TheRO; @trujillo2022penumbra].

Despite the fundamental importance of scientific software, its development and maintenance face persistent challenges in receiving appropriate academic recognition [@muna2016astropyproblem]. Software contributions are often relegated to acknowledgments rather than warranting authorship [@olivier_philippe_2019_2585783], and this lack of formal credit can significantly impact career advancement in academia [@Carver2022ASO; @biagioli2014scientific]. The academic community continues to grapple with developing appropriate systems for software citation and credit attribution that adequately reflect these contributions [@Merow2023BetterIA; @Westner2024CyclingOT; @Katz2020RecognizingTV].

Recent initiatives like the Contributor Roles Taxonomy (CRediT) have attempted to address these challenges by expanding academic authorship criteria to include specialized roles [@brand2015beyond], but have not fully resolved the issues surrounding software contribution recognition. While previous research has used CRediT and similar systems to understand research labor distribution [@Larivire2020InvestigatingTD; @lariviere2016contributorship; @sauermann2017authorship; @Li2023AreRC; @Lu2019CocontributorshipNA], these frameworks remain largely centered on traditional author lists. This traditional focus perpetuates existing problems, including historical and systematic bias, and relies heavily on self-reporting without external verification [@HAEUSSLER2013688; @gotzsche2007ghost; @chaoqun2021gendered]. The limitations of current contribution frameworks, combined with the growing importance of software in research, highlight the need for a systematic examination of how code contributions actually relate to academic recognition and impact.

These challenges raise several critical questions about the relationship between software development and academic recognition: How does the distribution of coding contributions within research teams relate to article impact? To what extent does sustained engagement in software development influence the likelihood of receiving authorship recognition? How do traditional academic roles align with actual code contributions? What are the career implications for researchers who regularly contribute code to research projects?

To address these questions, we developed a novel predictive model that enables systematic matching between scientific article authors and source code developer accounts. This approach was necessary due to the lack of standardized identifiers (like ORCID) for developers [@haak2012orcid] and inconsistencies in naming and email conventions across platforms. We leverage recent advances in transformer-based architectures and semantic embeddings to handle subtle variations in identity information [@Li2020DeepEM; @Brunner2020EntityMW].

By applying our model across a corpus of `{python} len(doc_repo_links)` paired research articles and repositories, we provide unique insight into the dynamics of code contribution within research teams. Our analysis moves beyond self-reporting to examine verifiable source-code repository commit histories, providing preliminary quantitative evidence of the exclusion of code contributors from academic authorship. While we find that code-contributing authors are associated with modest increases in article-level impact metrics (~4.5% increase in citations per code-contributing author), these effects are reduced or become statistically non-significant when controlling for factors such as domain, article type, and access status. Across every condition tested, first authors are consistently and significantly more likely to be code contributors compared to other author positions. However, when examining author-level impact metrics, we find a clear negative relationship between coding frequency and scholarly impact - authors who contribute code more frequently show progressively lower h-indices compared to their non-coding peers, a pattern that persists across various controls for author position, domain, and article type.

These findings begin to illuminate the complex and sometimes counterintuitive relationship between code contribution and scientific impact. While our results suggest that software development plays an important role in modern research, they also highlight potential tensions between technical contributions and traditional academic metrics. Our quantitative evidence could help inform ongoing discussions about academic credit systems, though more research is needed to fully understand how best to recognize and reward software development contributions in research.

# Background {#sec-background}

The relationship between scientific software development and academic credit systems represents a complex intersection of traditional academic practices and modern research requirements. While academic credit has historically focused on analytical, theoretical, and experimental contributions [@lariviere2016contributorship; @LIU2023101432], software development has long been viewed as purely technical rather than scholarly work. However, there is growing recognition that research software development requires deep domain expertise [@heroux2022researchsoftware; @Carver2022ASO], particularly as increased emphasis on large-scale data projects has amplified the need for sophisticated computational methods [@jin2015significance; @hampton2013big; @fan2014challenges].

Understanding the dynamics of scientific software development requires examining both team-level patterns and individual contributions. While existing research has extensively studied the impact of general team size and diversity on research outcomes [@Franceschet2010TheEO; @Larivire2014TeamSM; @alshebli2024china; @yang2024femaleled; @liu2021dominance; @naik2023impact], and software engineering literature has demonstrated correlations between team size and code quality [@Wyss2023NothingBut; @meirelles2010study], we lack systematic evidence of how code contribution patterns affect research impact. Given that technically sophisticated research projects often enable novel scientific insights and methodological advances, **we hypothesize that projects with more code contributors will receive higher citation counts, reflecting the value of technical contributions even when they go uncredited (H1)**.

Recent efforts to standardize contribution recognition, such as the CRediT taxonomy, have attempted to establish formal roles for software development [@Li2023AreRC; @Ding2021ANM]. However, these frameworks may not capture the full spectrum of technical contributions, particularly the relationship between sustained engagement in software development and authorship recognition. Repository histories provide a unique opportunity to examine this relationship [@ram2013git], leading us to hypothesize that longer project involvement increases the likelihood of authorship recognition. **Specifically, we expect that projects with longer commit durations will be associated with higher percentages of developers who receive authorship (H2)**.

Traditional academic authorship conventions carry specific expectations about intellectual contribution and project responsibilities. First authors typically bear primary responsibility for intellectual and experimental contributions [@lariviere2016contributorship; @Larivire2020InvestigatingTD; @Jnior2016PatternsOA], while corresponding authors maintain research artifacts and serve as primary points of contact. However, expectations regarding technical contributions vary significantly across academic disciplines [@smith2023technical]. **Given these conventions, we hypothesize that first authors (H3) and corresponding authors (H4) will show higher rates of code contribution compared to middle and last authors**.

The tension between software development and academic advancement is particularly acute given the field's reliance on traditional impact metrics. **We hypothesize that researchers who frequently contribute code will be associated with lower individual-level impact metrics compared to their non-coding peers (H5)**. This may occur through multiple mechanisms: lack of standardized software citation practices reduces recognition of technical contributions, theoretical advances garner more citations than methodological improvements, and version-specific iterative software publications lead to citation-splitting across multiple articles. This pattern could explain the career implications faced by researchers who prioritize coding [@hannay2009scientists; @heroux2022researchsoftware; @smith2019elevating; @cosden2022researchsoftware].
...
Our data collection process leverages multiple complementary sources of linked scientific articles and code repositories to ensure comprehensive coverage. Our dataset combines multiple sources:

- PLOS: Traditional research articles with code requirements
- JOSS and SoftwareX: Specialized software-focused publications
- Papers with Code (ArXiv): Capturing the pre-print landscape.

To reduce the complexity of dataset processing and enrichment, we filter out any article-source-code-repository pairs which store code somewhere other than GitHub. While this decision prioritizes processing simplicity, we acknowledge that while GitHub is the predominate host of much of scientific software, it is also stored and shared on other platforms [@trujillo2022penumbra].
...
1. Article Resolution and Metadata:
   - Use Semantic Scholar API for DOI resolution to find latest versions, particularly important for preprints
   - Collect publication metadata via OpenAlex API: open access status, domain, publication date
   - Gather author details: name, position, corresponding author status
   - Extract impact metrics: citation count, FWCI, h-index

2. Repository Information:
   - Use GitHub API to collect repository metadata: name, description, languages, creation date
   - Gather contributor details: username, name, email
   - Extract engagement metrics: star count, fork count, issue count

While the majority of our data is sourced from Papers with Code, our additional collection from PLOS, JOSS, and SoftwareX as well as the enrichment from GitHub and OpenAlex together form one of the largest and most comprehensively annotated collections of paired scientific articles and associated source-code repositories. In total, we collect and enrich data for `{python} original_doc_repo_links_len` article-repository pairs.
...
3. Developer Profile Completeness:
   - `{python} n_devs_with_name` (`{python} pct_devs_with_name`%) accounts have associated names
   - `{python} n_devs_with_email` (`{python} pct_devs_with_email`%) accounts have associated emails

This distribution of matches and varying completeness of developer profiles reflects the real-world challenges in identity matching across academic and software development platforms, requiring our model to handle both complete and partial information robustly.
...
- To optimize our predictive model for author-contributor matching, we evaluate a variety of Transformer-based base models and input features.
    - specifically, we fine-tune from three different base transformer models:
        - [deberta-v3-base](https://huggingface.co/microsoft/deberta-v3-base) [@he2021debertav3; @he2021deberta]
        - [bert-base-multilingual-cased](https://huggingface.co/bert-base-multilingual-cased) [@bert2018]
        - [distilbert-base-uncased](https://huggingface.co/distilbert/distilbert-base-uncased) [@Sanh2019DistilBERTAD]
    - these three models are all variations or built-upon BERT, and while significant time has passed since BERT was first introduced, BERT-based models remain a strong base for many NLP tasks across a number of domains while being relatively "small" compared to the much larger decoder transformer relatives (GPT, Llama, etc.) [@tran2024deberta; @yu2024semantic; @jeong2022scideberta]
...
- After extensive model comparison we find that fine-tuning from [Microsoft's deberta-v3-base](https://huggingface.co/microsoft/deberta-v3-base) and including the developer's username and name achieves the best performance for author-developer matching.
    - our best model achieves a binary F1 score of `{python} best_model_f1`, with an accuracy of `{python} best_model_acc`, precision of `{python} best_model_prec`, and recall of `{python} best_model_rec`
...
- To enable future research, we have made our trained model and supporting application library publicly available.
    - Python library implementation: [sci-soft-models](github.com/evamaxfield/sci-soft-models)
    - HuggingFace model deployment: [dev-author-em-clf](https://huggingface.co/evamxb/dev-author-em-clf)
...
- Our final dataset provides unprecedented scale and scope for analyzing the relationship between academic authorship and software development contributions.
    ...
    - From the `{python} total_authors` distinct authors and `{python} total_devs` distinct developer accounts we are able to create `{python} len(researcher_dev_links)` annotated author-developer pairs
...
for _, row in docs_w_1_citation.iterrows():
    ...
    # Drop any dev researcher links that have less than 90% confidence
    dev_researcher_links = dev_researcher_links.loc[
        dev_researcher_links["predictive_model_confidence"] >= 0.9
    ]
...
- We begin by measuring the distributions of different coding and non-coding contributors across all of the article-code-repository pairs within our dataset.
    ...
    - within our dataset, we find that papers on average have `{python} total_auths` total authors, `{python} total_auth_non_devs` non-code-contributing authors, `{python} total_auth_devs` code-contributing authors, and `{python} total_non_auth_devs` code-contributing non-authors (see @fig-contributor-type-distribution for a visualization of these distributions).
    - @tbl-team-composition-counts provides a detailed breakdown of these distributions by domain, article type, and open access status.
	- Our finding of, on average, only a single code-contributing author on a paper, is similar to previous work in understanding distributions of labor in knowledge production from @Larivire2020InvestigatingTD which found that the CRediT tasks of "Data Curation", "Formal Analysis", "Visualization", and "Software" were all predominantly performed by a first author.
...
```{python}
#| label: fig-contributor-type-distribution
#| fig-cap: "Distribution of the number of Total Authors, Non-Code-Contributing Authors, Code-Contributing Authors, and Code-Contributing Non-Authors across all article-repository pairs. Only includes article-repository pairs with a most recent commit no later than 90 days after publication."
...
- Next we investigate how these distributions have changed over time and how they change by the total number of authors on a paper.
	- @fig-contributor-type-by-time-and-size shows thats both, the mean number of code-contributing authors and code-contributing non-authors has remained relatively stable over time and across different total author sizes.
...
- Building upon previous work which discuss the effects of team size and team diversity on scientific impact and software quality (see @sec-background), we examine how the number of code contributors within a research team may be associated with an article's research impact.
...
- However, after analyzing our data, we find few significant associations between the number of code-contributing authors and non-authors, and article citations.
  - Without controlling for any domain, open access, or article type differences (@tbl-article-composition-overall), we find a positive association between the number of code-contributing authors and article citations with each code-contributing author being associated with a 4.5% increase in article citations (p < 0.001).
  - Controlling for article type (@tbl-article-composition-type), we find that for preprints, each code-contributing non-author is associated with a statistically significant 2.9% decrease in expected citations, holding other variables constant (p < 0.01).
...
- Building upon previous work examining standardized contribution frameworks and authorship attribution practices (see @sec-background), we investigate how project duration may influence the distribution of coding roles between authors and non-authors.
...
- However, our analysis finds no evidence to support this hypothesis:
  - When examining the relationship between a repository's commit duration and the percentage of developers who receive authorship recognition, we find no significant correlation (r = `{python} pearson_rho`, p = n.s.).
...
- Our analysis supports this hypothesis:
  ...
  - @tbl-post-hoc-tests-on-author-positions provides a detailed breakdown of Bonferronic-correct p-values from post-hoc binomial tests against the counts of author positions of code-contributing authors.
...
- However, our analysis reveals patterns contrary to this hypothesis:
  - Both corresponding and non-corresponding authors are statistically significantly less likely to be code contributors than would be expected by chance (p < 0.001) in most scenarios (@tbl-post-hoc-tests-on-corresponding-status).
...
for control_var, control_display_name in control_vars.items():
    ...
    # Handle position sorter
    if control_var == "common_author_position":
        count_table["Subset"] = pd.Categorical(
            count_table["Subset"],
            categories=position_sorter,
            ordered=True,
        )
        ...
        if i == 0:
            count_portion_html.append(
                count_piece_inital_row_template.format(
                    n_rows=len(count_table),
                    row_name=key,
                    value_name=control_value.title(),
...
- Building upon previous work examining career implications for researchers who prioritize software development (see @sec-background), we investigate how varying levels of code contribution relate to scholarly impact through citation metrics.
...
- While we can't confirm the underlying mechanisms, we do find strong evidence to support our general claim that code-contributing researchers are associated with lower h-indices compared to their non-coding peers.
    - Without any controls (@tbl-researcher-coding-status-no-control), we find increasingly negative h-index effects as coding frequency increases compared to non-coding authors with scientists who occasionally make code contributions being associated with ~27.3% lower h-indices than their non-coding counterparts (p < 0.001), followed by ~53.5% lower h-indices for majority coders (p < 0.001), and ~62.1% lower h-indices for always coding authors (p < 0.001).
    - When controlling for author position (@tbl-researcher-coding-status-author-position), we find again find a general pattern that more frequent code contribution is associated with reduced h-indices compared to non-coding peers with a few exceptions.
