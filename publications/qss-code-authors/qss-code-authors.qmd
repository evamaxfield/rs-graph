...
```{python}
# LaTeX templates
model_results_row_template = """    {bold_start}{var_name} & \\cellcolor{{{color}}}{exp_coef:.6f} & \\cellcolor{{{color}}}{coef:.4f} & \\cellcolor{{{color}}}{z:.3f} & \\cellcolor{{{color}}}{p:.3f} & \\cellcolor{{{color}}}{ci_low:.3f} & \\cellcolor{{{color}}}{ci_high:.3f}{stars}{bold_end} \\\\"""
...
\\begin{tabular}{l*{6}{r}}
\\toprule
\\textbf{Variable} & \\textbf{exp(coef)} & \\textbf{coef} & \\textbf{z} & \\textbf{P>|z|} & \\multicolumn{2}{c}{\\textbf{[0.025 0.975]}} \\\\
\\midrule"""
...
# Function to convert model to printable latex
def convert_model_results_to_printable_pdf_ready(model: sm.GLM, tbl_cap: str) -> str:
    # Get only the dataframe
    summary_simple_tab = model.summary().tables[1]
    model_results_df = pd.read_html(summary_simple_tab.as_html(), header=0, index_col=0)[0]

    # Add exponentiated coef
    model_results_df["exp(coef)"] = np.exp(model_results_df["coef"])

    # Keep only the "coef", "z", "P>|z|", and "[0.025', '0.975]" columns
    ...
    tbl_cap_latex = "\\caption{" + tbl_cap + "}"

    # Combine all parts
    regression_table_latex = f"""
    \\centering
    {tbl_cap_latex}
    {model_results_table_header}
    ...
    regression_table_latex = "\\begin{table}\n" + regression_table_latex

    with open("output.tex", "w") as f:
        f.write(regression_table_latex)

    print(regression_table_latex)
...
IPython.display.Latex(
    convert_model_results_to_printable_pdf_ready(
...
# IPython.display.display(exp_results.to_html(index=False))
```
