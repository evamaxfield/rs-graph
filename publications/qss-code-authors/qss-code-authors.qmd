...
- Our trained entity-matching model enables comprehensive identification of author-developer relationships while accounting for the complex realities of academic software development practices.
    - in practice, to complete our dataset, we apply our trained model to all possible author and developer-account combinations within each article-repository pair
...
- To enrich our pre-existing dataset, we apply our trained predictive model across pairs of authors and developer accounts.
    ...
    - we find `{python} len(researcher_dev_links)` unique author-developer account pairs
...
- We begin by measuring the distributions of different coding and non-coding contributors across all of the article-code-repository pairs within our dataset.
    - individuals in our dataset can fall into three categories:
        - code-contributing authors (Code Contrib. Authors): authors for which our model predicted a match with at least one developer account which contributed code to the associated repository
        - non-code-contributing authors (Non-Code Contrib. Authors): authors for which our model did not predict any matches with developer accouts which contributed code to the associated repository
        - code-contributing non-authors (Code Contrib. Non-Authors): developer accounts which contributed code to the associated repository but were not predicted to be a match with any author
    ...
    - However, we also find that, on average, papers have one code-contributing non-author, which may suggest that there are significant amounts of software development work being done by individuals who are not credited as authors on the paper.

```{python}
#| label: fig-contributor-type-distribution
#| fig-cap: "Distribution of the number of total authors, code-contributing authors, non-code-contributing authors, and code-contributing non-authors across all article-repository pairs. Only includes article-repository pairs with a most recent commit no later than 90 days after publication."
...
for control_var, control_display_name in control_vars.items():
    ...
    count_table = count_table.rename(
        columns={
            "n_authors": "Authors",
            "n_author_non_devs": "Non-Code Cntrb. Auth.",
            "n_author_devs": "Code Cntrb. Auth.",
            "n_non_author_devs": "Code Cntrb. Non-Auth.",
        }
    )
...
<table>
  <tr>
    ...
    <th><b>Non-Code Cntrb. Auth.</b></th>
    <th><b>Code Cntrb. Auth.</b></th>
    <th><b>Code Cntrb. Non-Auth.</b></th>
  </tr>
...
\\begin{tabular}{llrrr}
\\toprule
\\textbf{Control} & \\textbf{Subset} & \\textbf{Total Authors} & \\textbf{Non-Code Cntrb. Auth.} & \\textbf{Code Cntrb.\\ Auth.} & \\textbf{Code Cntrb. Non-Auth.} \\\\
\\midrule"""
...
- Next we investigate how these distributions have changed over time and how they change by the total number of authors on a paper.
	- @fig-contributor-type-over-time shows that while the mean number of code-contributing authors has remained relatively stable over time, the mean number of code-contributing non-authors has increased over time.
	- @fig-contributor-type-over-team-size shows that the mean number of code-contributing authors remaines relatively stable regardless of the total number of authors on a paper, while the mean number of code-contributing non-authors increases as the total number of authors on a paper increases.
    - both findings suggest that the number of code-contributing non-authors is increasing over time and that larger authorship articles may still not be crediting all individuals who contribute to the software development process.

```{python}
#| label: fig-contributor-type-over-time
#| fig-cap: "Mean number of non-code-contributing authors, code-contributing authors, and code-contributing non-authors over time. Only includes article-repository pairs with a most recent commit no later than 90 days after publication and only includes publication years with 50 or more articles."
...
```{python}
#| label: fig-contributor-type-over-team-size
#| fig-cap: "Mean number of non-code-contributing authors, code-contributing authors, and code-contributing non-authors by the total number of authors on a paper. Only includes article-repository pairs with a most recent commit no later than 90 days after publication."

# Make a copy for team size comparison
team_comp_for_team_size = team_comp_no_push_after_pub.copy()

# Rename columns
team_comp_for_team_size = team_comp_for_team_size.rename(
    columns={
        "n_authors": "Total Authors",
        "n_author_devs": "Code Contrib. Authors",
        "n_author_non_devs": "Non-Code Contrib. Authors",
...
# Melt again but separate out "n_authors" from the rest
team_comp_for_team_size_melted = team_comp_for_team_size.melt(
    id_vars=[
        "document_id",
        "Total Authors",
    ],
...
# Plot
sns.relplot(
    data=team_comp_for_team_size_melted,
    x="Total Authors",
    y="count",
    hue="Contributor Type",
    hue_order=[
