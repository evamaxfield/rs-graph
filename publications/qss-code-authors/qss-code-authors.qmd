...
source_counts_df = pd.DataFrame([
  *source_stats,
]).set_index("dataset").sort_values("n_article_repo_pairs", ascending=False)
...
<!-- ```{python}
# SQL Statements
article_repo_pairs_for_field = """
SELECT COUNT(document_repository_link.id)
FROM document_repository_link
JOIN document
  ON document_repository_link.document_id=document.id
JOIN document_topic
  ON document.id=document_topic.document_id
JOIN topic
  ON document_topic.topic_id=topic.id
WHERE topic.field_name='{field_name}'
AND document_topic.id = (
    SELECT document_topic.id
    FROM document_topic 
    WHERE document_topic.document_id = document.id
    ORDER BY document_topic.score DESC
    LIMIT 1
)
""".strip()
...
AND document_topic.id = (
    SELECT document_topic.id
    FROM document_topic 
    WHERE document_topic.document_id = document_contributor.document_id
    ORDER BY document_topic.score DESC
    LIMIT 1
)
""".strip()

developers_for_field = """
SELECT COUNT(developer_account.id)
FROM developer_account
JOIN repository_contributor
  ON developer_account.id=repository_contributor.developer_account_id
JOIN document_repository_link
  ON repository_contributor.repository_id=document_repository_link.repository_id
JOIN document_topic
  ON document_repository_link.document_id=document_topic.document_id
JOIN topic
  ON document_topic.topic_id=topic.id
WHERE topic.field_name='{field_name}'
AND document_topic.id = (
    SELECT document_topic.id
    FROM document_topic 
    WHERE document_topic.document_id = document_repository_link.document_id
    ORDER BY document_topic.score DESC
    LIMIT 1
)
""".strip()
...
with Session(prod_db) as session:
  ...
  field_names = [
    field[0]
    for field in session.exec(text("SELECT DISTINCT field_name FROM topic"))
  ]

  # Create a lookup table of stats per field
  field_stats = []
  ...
  for field in field_names:
    n_article_repo_pairs = session.exec(
      text(
        article_repo_pairs_for_field.format(
          field_name=field,
        )
      )
    ).first()[0]
...
field_counts_df = pd.DataFrame([
  *field_stats,
]).set_index("field").sort_values("n_article_repo_pairs", ascending=False)
...
with Session(prod_db) as session:
  ...
  # Create a lookup table of stats per domain
  domain_stats = []
  ...
  for domain in domains:
    n_article_repo_pairs = session.exec(
      text(
        article_repo_pairs_for_domain.format(
          domain_name=domain,
        )
      )
    ).first()[0]
...
domain_counts_df = pd.DataFrame([
  *domain_stats,
]).set_index("domain").sort_values("n_article_repo_pairs", ascending=False)
...
```{python}
# SQL Statements
article_repo_pairs_for_document_type = """
SELECT COUNT(document_repository_link.id)
FROM document_repository_link
JOIN document
  ON document_repository_link.document_id=document.id
...
with Session(prod_db) as session:
  ...
  doc_types = [
    dt[0]
    for dt in session.exec(text("SELECT DISTINCT document_type FROM document"))
  ]

  # Create a lookup table of stats per doc type
  doc_type_stats = []

  # For each doc type, get stats
  for doc_type in doc_types:
    n_article_repo_pairs = session.exec(
      text(
        article_repo_pairs_for_document_type.format(
    ...
    n_authors = session.exec(
      text(
        authors_for_document_type.format(
          document_type=doc_type,
        )
      )
    ).first()[0]
    ...
    n_devs = session.exec(
      text(
        developers_for_document_type.format(
          document_type=doc_type,
        )
      )
    ).first()[0]
...
# Create dataframe using source names as index
# Columns as count type
doc_type_counts_df = pd.DataFrame([
  *doc_type_stats,
]).set_index("document_type").sort_values("n_article_repo_pairs", ascending=False)
...
```{python}
# SQL Statements
article_repo_pairs_for_oa_type = """
SELECT COUNT(document_repository_link.id)
FROM document_repository_link
JOIN document
  ON document_repository_link.document_id=document.id
WHERE document.is_open_access={oa_bool}
""".strip()
...
# Execute SQL to get counts
with Session(prod_db) as session:
  # Create a lookup table of stats per oa flag
  oa_stats = []

  # For each doc type, get stats
  for oa_value, oa_bool in {"open": 1, "closed": 0}.items():
    n_article_repo_pairs = session.exec(
      text(
        article_repo_pairs_for_oa_type.format(
          oa_bool=oa_bool,
        )
      )
    ).first()[0]
...
# Create dataframe using source names as index
# Columns as count type
oa_counts_df = pd.DataFrame([
  *oa_stats,
]).set_index("open_access").sort_values("n_article_repo_pairs", ascending=False)
