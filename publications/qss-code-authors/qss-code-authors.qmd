...
[^2]: Our dataset is made available via Harvard Dataverse at [https://doi.org/10.7910/DVN/KPYVI1](https://doi.org/10.7910/DVN/KPYVI1). Portions of the dataset are restricted to keep researcher and developer information private. The full dataset is available upon request via the Harvard Dataverse. For details on how to interact with this dataset, see our documentation at [https://github.com/evamaxfield/rs-graph](https://github.com/evamaxfield/rs-graph).
...
_H1: The number of individuals contributing code to a publication's associated repository is positively correlated with the article's citation count._
...
_H2a: First authors have higher rates of code contribution compared to authors in other positions._

_H2b: Corresponding authors have higher rates of code contribution compared to non-corresponding authors._
...
The resulting dataset represents, to our knowledge, the first large-scale collection of linked article-repository and author-developer-account pairs, particularly in the physical sciences. Specifically, our dataset contains almost 140,000 article-repository pairs, with nearly 300,000 distinct authors and more than 150,000 distinct developer accounts. From these distinct entities, we identify almost 110,000 author-developer relationships. A detailed breakdown of these counts by data source, domain, document type, and open access status is available in @tbl-rs-graph-overall-counts.

Our compiled dataset is made available via Harvard Dataverse at [https://doi.org/10.7910/DVN/KPYVI1](https://doi.org/10.7910/DVN/KPYVI1). Portions of the dataset are restricted to keep researcher and developer information private. The full dataset is available upon request via the Harvard Dataverse. For details on how to interact with this dataset, see our documentation at [https://github.com/evamaxfield/rs-graph](https://github.com/evamaxfield/rs-graph).
...
Understanding the composition and dynamics of software development teams provides essential context for analyzing how code contributions relate to scientific recognition and impact. To ensure reliable analysis, we focus on a subset of our article-repository pairs that meet several filtering conditions. First, we require that each article-repository pair must have at least one article citation, which helps ensure the research has received some minimal level of engagement from the scientific community. Next, we require that repository commit activity must stop prior to 90 days past the date of article publication. This ensures we aren't including projects which may add additional code contributors later, while allowing a grace period in which repositories are updated with additional documentation and publication information. We then subset the data to only include article-repository pairs with research teams of typical size by removing those with fewer than 3 authors and more than `{python} int(n_authors_97th_percentile)` authors, the 97th percentile for research team size. Finally, we filter out any author-developer pairs associated with these projects which have a predictive model confidence of less than 0.97 to ensure that we are only including high-confidence matches (TODO: add footnote reference to appendix plot). This filtering process results in a dataset of `{python} len(team_comp_no_push_after_pub)` article-repository pairs.
...
Based on these findings, we _**partially accept**_ our hypothesis (_H1_) that "the number of individuals contributing code to a publication's associated repository is positively correlated with the article's citation count." This acceptance is qualified by several important nuances: the relationship is statistically significant but modest in magnitude, and differs substantially between research articles (positive association) and preprints (negative association for non-author code contributors). These variations suggest that the relationship between code contributions and citation impact is context-dependent and more complex than initially hypothesized.
...
Based on these findings, we _**accept**_ our hypothesis (_H3_) that "the frequency at which a researcher contributes code to their research projects is negatively correlated with their h-index." Our analysis demonstrates a clear and statistically significant negative relationship between coding frequency and scholarly impact as measured by h-index. This relationship was robust across multiple analytical controls including author position, research domain, and article type. What makes these results particularly striking is that each of our models includes publication count as an input feature, suggesting that these h-index reductions persist even when accounting for total research output.
