...
The scientific community has attempted to address this recognition gap through initiatives like the Contributor Roles Taxonomy (CRediT). CRediT supports more inclusive authorship criteria for specialized contribution roles, including software development [@brand2015beyond; @Li2023AreRC; @Lu2019CocontributorshipNA]. Despite these efforts, current frameworks remain centered on traditional author lists and may perpetuate historical biases about what consitutes meaningful scientific contribution [@HAEUSSLER2013688; @gotzsche2007ghost; @chaoqun2021gendered]. Simultaneously, researchers have also advocated for software citation systems that better reflect the unique contributions of research software developers, though these systems often remain separate from formal authorship recognition [@Merow2023BetterIA; @Westner2024CyclingOT; @Katz2020RecognizingTV].
...
Perhaps most striking is our finding that `{python} n_papers_w_non_auth_devs` papers (`{python} pct_papers_w_non_auth_devs`%) have at least one code contributor who was not matched to any article author. Within this substantial subset of papers, we found an average of `{python} mean_non_author_devs_for_papers_w_non_auth_devs` un-matched code contributors per paper. 

To better understand the nature of these contributions, we conducted a brief analysis of a random sample of 200 code-contributing non-authors (see Appendix @sec-appendix-additional-cc-na-analysis for full methodology). Our analysis revealed that approximately 39% represent true non-authors who made legitimate contributions but should not receive authorship credit, while 30.5% appear to be missed classifications who likely should have been matched to authors. The remaining 30.5% were unclear due to limited profile information. Notably, among the true non-authors, 76% (59 of 78) contributed actual code rather than just documentation, with the top quartile contributing approximately 10% of total repository commits and 14% of absolute code changes. This suggests that a substantial portion of unrecognized contributors are making meaningful technical contributions to scientific software.

Our finding that on average, each paper has only a single code-contributing author aligns with previous research by @Larivire2020InvestigatingTD, showing that technical tasks like data curation, formal analysis, visualization, and software development typically fall to first authors. However, our finding that over a quarter of papers have unrecognized code contributors—many making substantial technical contributions—suggests a more complex dynamic between software development and authorship recognition than previously documented.
