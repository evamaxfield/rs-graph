...
team_members_per_doc = (
    document_contributors.groupby("document_id")
    .size()
    .reset_index()
...
document_repository_contribs_lookup = (
    ...
    .size()
    .reset_index()
    .rename(columns={0: "n_code_contributors"})
)
...
team_members_per_doc = (
    ...
    .reset_index()
    .rename(columns={"developer_account_id": "n_code_contributing_authors"})
)

team_members_per_doc["n_code_contributing_non_authors"] = (
    team_members_per_doc["n_code_contributors"]
    - team_members_per_doc["n_code_contributing_authors"]
)
...
team_members_per_doc["total_team_size"] = (
    team_members_per_doc["n_authors"]
    + team_members_per_doc["n_code_contributing_non_authors"]
)

# Add columns for each of the counts as a percentage of the total team size
# team_members_per_doc["percent_authors"] = (
#     team_members_per_doc["n_authors"] / team_members_per_doc["total_team_size"]
# )
# team_members_per_doc["percent_code_contributors"] = (
#     team_members_per_doc["n_code_contributors"]
#     / team_members_per_doc["total_team_size"]
# )
# team_members_per_doc["percent_code_contributing_authors"] = (
#     team_members_per_doc["n_code_contributing_authors"]
#     / team_members_per_doc["total_team_size"]
# )
# team_members_per_doc["percent_code_contributing_non_authors"] = (
#     team_members_per_doc["n_code_contributing_non_authors"]
#     / team_members_per_doc["total_team_size"]
# )

# # We have essentially created a dummy-trap here
# # we need to drop two of the columns
# team_members_per_doc = team_members_per_doc.drop(
#     columns=[
#         "n_authors",
#         "percent_authors",
...
# Create dummies for document_type, domain
team_members_per_doc_with_dummies = pd.get_dummies(
    team_members_per_doc,
    columns=["domain_name"],
    drop_first=True,
)
team_members_per_doc_with_dummies = team_members_per_doc_with_dummies.drop(
    columns=["document_type"]
)
# team_members_per_doc_with_dummies = team_members_per_doc.drop(
#     columns=["document_type", "domain_name", "years_since_publication"]
# )
```

```{python}
#| label: doc-model-cited-by-count
#| tbl-cap: "Model summary from Poisson Generalized Linear Model for Total Article Citations."
...
data_for_cited_by_count = data_for_cited_by_count[
    data_for_cited_by_count["cited_by_count"]
    < data_for_cited_by_count["cited_by_count"].quantile(0.97)
]
data_for_cited_by_count = data_for_cited_by_count[
    data_for_cited_by_count["cited_by_count"]
    > data_for_cited_by_count["cited_by_count"].quantile(0.03)
]
...
# Create interaction effects for
# percent_code_contributors and percent_code_contributing_authors
# with document_type and domain_name
for col in X.columns:
    if any([variable in col for variable in ["domain_name", "document_type"]]):
        # X[f"percent_code_contributors * {col}"] = X["percent_code_contributors"] * X[col]
        X[f"percent_code_contributing_authors * {col}"] = X["percent_code_contributing_authors"] * X[col]
...
```{python}
#| label: doc-model-fwci
#| tbl-cap: "Model summary from Poisson Generalized Linear Model for Field-Weighted Citation Impact."
...
```{python}
#| label: doc-model-cited-by-percentile-year-min
#| tbl-cap: "Model summary from Poisson Generalized Linear Model for Cited by Percentile Year Minimum."
...
researcher_docs = (
    ...
    .rename(
        columns={
            ...
            "is_corresponding": "is_corresponding_author",
        },
    )
...
researcher_position_counts = (
    researcher_docs.groupby("researcher_id")["author_position"]
    .value_counts()
    .unstack(fill_value=0)
)
researcher_position_counts["most_frequent_author_position"] = (
    researcher_position_counts.idxmax(axis=1)
)
...
researcher_is_corresponding_counts = (
    researcher_docs.groupby("researcher_id")["is_corresponding_author"]
    .value_counts()
    .unstack(fill_value=0)
)
researcher_is_corresponding_counts["most_frequent_corresponding_author_status"] = (
    researcher_is_corresponding_counts.idxmax(axis=1)
)
...
document_is_open_access_counts = (
    researcher_docs.groupby("researcher_id")["is_open_access_document"]
    .value_counts()
    .unstack(fill_value=0)
)
document_is_open_access_counts["most_frequent_document_open_access_status"] = (
    document_is_open_access_counts.idxmax(axis=1)
)
...
document_type_counts = (
    researcher_docs.groupby("researcher_id")["document_type"]
    .value_counts()
    .unstack(fill_value=0)
)
document_type_counts["most_frequent_document_type"] = document_type_counts.idxmax(
    axis=1
)
...
domain_counts = (
    researcher_docs.groupby("researcher_id")["domain_name"]
    .value_counts()
    .unstack(fill_value=0)
)
...
dataset_counts = (
    researcher_docs.groupby("researcher_id")["dataset"]
    .value_counts()
    .unstack(fill_value=0)
)
...
researcher_counts["n_documents"] = researcher_counts["researcher_id"].map(
    researcher_docs_count
)
...
researcher_coded_counts = (
    researcher_coded_counts.groupby("researcher_id")["document_id"]
    .count()
    .to_frame()
    .reset_index()
    .rename(columns={"document_id": "n_times_coded"})
)
...
researcher_counts["most_frequent_coding_status"] = researcher_counts[
    "n_times_coded"
] >= (0.5 * researcher_counts["n_documents"])
...
for col in [
    ...
    researcher_counts[col] = researcher_counts[col].apply(
        lambda x: 1 if x == "is" else 0
    )
...
```{python}
#| label: researcher-model-cited-by-count
#| tbl-cap: "Model summary from Poisson Generalized Linear Model for Total Researcher Citations."
...
```{python}
#| label: researcher-model-h-index
#| tbl-cap: "Model summary from Poisson Generalized Linear Model for Researcher h-index."

X = X.drop(columns=[col for col in X.columns if "n_times" in col])
X = X.drop(columns=[col for col in X.columns if "dataset_" in col])
X = X.drop(columns=[col for col in X.columns if "corresponding_" in col])
X = X.drop(columns=[col for col in X.columns if "open_access_status" in col])
X = X.drop(columns=[col for col in X.columns if "document_type" in col])
X = X.drop(columns=[col for col in X.columns if "n_documents" in col])

# Create interactions between "most_frequent_coding_status" and all other "most_frequent" columns
for col in X.columns:
    if "most_frequent" in col and col != "most_frequent_coding_status":
        X[f"most_frequent_coding_status * {col}"] = X["most_frequent_coding_status"] * X[col]
...
```{python}
#| label: researcher-model-i10-index
#| tbl-cap: "Model summary from Poisson Generalized Linear Model for Researcher i10-index."
...
```{python}
#| label: researcher-model-two-year-mean-citedness
#| tbl-cap: "Model summary from Poisson Generalized Linear Model for Researcher Two-Year Mean Citedness."
